...
assert NVIDIA_API_KEY, "❌ Missing NVIDIA_API_KEY"
...
def evaluate_question(question: str, model_cfg: dict):
...
def run_model(model_name: str, model_cfg: dict):
    print(f"\n🚀 Running model: {model_name}")
    dataset = []

    for i, question in enumerate(QUESTIONS):
        result = evaluate_question(question, model_cfg)
        if result is None:
            continue
...
if __name__ == "__main__":
    for model_name, model_cfg in MODEL_CONFIGS.items():
        run_model(model_name, model_cfg)