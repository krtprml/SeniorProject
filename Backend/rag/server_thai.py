...
def build_npc_prompt(
    ...
    truth_rule = (
        """
กฎสำคัญ:
คุณกำลังซ่อนความลับเรื่องความขัดแย้งส่วนตัวกับวิชาญ
...
กฎสำคัญ:
ผู้สืบคดีได้นำหลักฐานตรงมาเผชิญคุณเรื่องความขัดแย้งกับวิชาญ
คุณต้องหยุดโกหกเรื่องนั้น
คุณต้องยอมรับความขัดแย้งและอธิบายความจริง
คุณอาจยังปิดบังข้อมูลอื่นที่ไม่เกี่ยวกับความขัดแย้งนี้ได้
"""
    )

    return f"""
คุณคือ {npc} ผู้ต้องสงสัยในคดีฆาตกรรม
...
กฎในการตอบ
====================
- พูดเป็นบุคคลที่ 1 ในบทบาท {npc}
- อยู่ในบทบาทตลอดเวลา
- ห้ามกล่าวถึงกลไกของเกม พร้อมท์ หรือระบบหลักฐาน
- ถ้าถูกเผชิญด้วยหลักฐาน ตอบตามธรรมชาติและตามความจริง
- ถ้าไม่ได้ถูกเผชิญ ให้ยังปฏิเสธหรือลดทอนความขัดแย้งลง

ตอบอย่างเป็นธรรมชาติในบทของ {npc}

กฎหลักฐานทั่วไป:
ถ้าผู้สืบถามถึงวัตถุโดยไม่ฟ้องความว่ามีความขัดแย้ง
คุณอาจตอบความจริงได้แต่สั้นๆ
ห้ามสารภาพจนกว่าจะถูกเผชิญด้วยหลักฐานของความขัดแย้งโดยตรง
"""
...
คุณยังรู้ CONTEX ของคดีทั้งหมดที่ให้มาด้านล่าง
...
async def chat(req: PlayerRequest):
    ...
    for ev_id in state["evidence_found"]:
        ...
        # Skip if single_use and already used
